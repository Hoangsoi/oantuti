...
import { Swords, RefreshCw, Plus, Bot, Coins, ShieldAlert, Sparkles, ArrowLeft } from 'lucide-react';
...
export const LobbyPage: React.FC<LobbyPageProps> = ({
  ...
  useEffect(() => {
    fetchRooms();
    // Auto polling every 5 seconds for live room updates
  ...
  const handleJoin = async (roomCode: string) => {
    ...
    try {
      const joinedRoom = await api.joinRoom(roomCode);
  ...
  return (
    <div className="flex flex-col min-h-screen px-4 py-6 max-w-md mx-auto pb-24 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={onBackHome}
          className="px-3 py-1.5 rounded-xl bg-slate-800 text-slate-300 text-xs font-bold border border-slate-700 flex items-center gap-1"
        >
          ...
          <div className="space-y-2.5">
            {filteredRooms.map((room) => {
              ...
              return (
                <div
                  key={room.id}
                  className="card-glass p-3.5 flex items-center justify-between border-slate-700/80 hover:border-amber-500/50 transition-all bg-slate-900/90"
                >
                  {/* Host Info & Room Code */}
